...
var keysAreBeingReloaded = (only, except, keys2) => {
  if (only.length === 0 && except.length === 0) {
    return true;
  }
  if (only.length > 0) {
    return keys2.some((key2) => only.includes(key2));
  }
  return keys2.some((key2) => !except.includes(key2));
};
var deferred_default = defineComponent({
  ...
  setup(props, { slots }) {
    ...
    onMounted(() => {
      const keys2 = Array.isArray(props.data) ? props.data : [props.data];
      removeStartListener = router.on("start", (e) => {
        const visit = e.detail.visit;
        if (visit.preserveState === true && isSameUrlWithoutQueryOrHash(visit.url, window.location) && keysAreBeingReloaded(visit.only, visit.except, keys2)) {
    ...
    return () => {
      ...
      return keys2.every((key2) => get(page22.props, key2) !== void 0) ? (_a2 = slots.default) == null ? void 0 : _a2.call(slots, { reloading: reloading.value }) : slots.fallback({});
    };
  }
});
var noop4 = () => void 0;
